...
class XConsoleAuthClient:
    ...
    def _scrape_action_id(self, html: str) -> str:
        ...
        # 注意：x.ai 部署的 chunk URL 可能带 ?dpl=... 查询参数，不能用 .js" 结尾的严格匹配
        js_urls = list(set(re.findall(r'src="(/_next/static/chunks/[^"]+\.js(?:\?[^"]*)?)"', html)))
        if not js_urls:
            js_urls = list(set(re.findall(r'(/_next/static/chunks/[^"\s>]+\.js(?:\?[^"\s>]*)?)', html)))
        if self.debug:
    ...
    def create_account(self, *, email: str, given_name: str, family_name: str,
        ...
        # 实测：真成功响应为完整 RSC flight(几十KB~100KB+，含 sso 证据)；
        # 71 字节的空响应是风控/建号被拒信号(HTTP 200 但无会话无跳转)，
        # 不判成功，否则后续 sso 全是空等、CreateSession 必报 invalid-credentials。
        tiny_empty = (
            0 < len(rsc_body) < 1000
            and "sso=" not in rsc_body
            and "set-cookie?q=" not in rsc_body
        )
        if status == 200 and not hard_error and not tiny_empty:
            ok = True
        else:
