...
def save_grok_pending(email="", password=""):
    """建号成功但 sso 暂未取到时落盘账号(待补 sso)，避免浪费建好的号。返回 True/False。"""
    e = _s(email)
    if not e or not _EMAIL_RE.match(e):
        return False
    pdir = os.path.join(TOKEN_OUTPUT_DIR, "grok", "pending")
    os.makedirs(pdir, exist_ok=True)
    name = _safe_email_name(e)
    path = os.path.join(pdir, f"{name}.json")
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"email": e, "password": _s(password), "ts": int(time.time()),
                   "status": "sso_pending",
                   "note": "账号已建，sso 待补(可用密码登录补 sso)"},
                  f, indent=2, ensure_ascii=False)
    print(f"  [grok] pending account saved: {path}")
    return True
