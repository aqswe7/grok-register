...
def _sub2api_group_id(origin, token, group, platform, timeout=DEFAULT_TIMEOUT, admin_key=""):
    target = str(group or platform).strip().lower()
    groups = _sub2api_request(
        origin, "/api/v1/admin/groups/all", token=token, timeout=timeout, admin_key=admin_key
    )
...
def _create_sub2api_grok_oauth(origin, token, group_id, credentials, account_email,
                               concurrency, priority, timeout, admin_key=""):
...
def upload_sub2api_grok(base_url, email, password, group, sso, account_email="",
    ...
    认证优先使用 admin_key（x-api-key 直连）；未提供时回退账密登录。
    """
    try:
        origin = _origin(base_url)
        ...
        if not admin_key:
            token = _sub2api_login(origin, email, password, timeout=timeout)
        group_id = _sub2api_group_id(
            origin, token, group or "grok", "grok", timeout=timeout, admin_key=admin_key
        )
        body = {
            "sso_tokens": [sso],
            "group_ids": [group_id],
