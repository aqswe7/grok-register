...
try:
    from config import (
        ...
        REMAIL_BASE_URL, REMAIL_API_KEY, REMAIL_PROJECT_ID, REMAIL_PRODUCT_ID,
    )
...
def normalize_provider(provider=None, base_url=""):
    """归一化 provider 名：moemail | yyds | gptmail | cfmail | custom | remail。
    provider 显式给出优先；否则从 base_url 域名特征推断；再兜底 config 默认。"""
    p = (provider or "").strip().lower()
    if p in ("moemail", "yyds", "gptmail", "cfmail", "custom", "remail"):
        return p
    b = (base_url or "").lower()
    ...
    if "temp-email" in b or "awsl" in b:
        return "cfmail"
    if "remail" in b or "aishop6" in b:
        return "remail"
    dflt = (TEMP_EMAIL_PROVIDER or "gptmail").strip().lower()
    return dflt if dflt in ("moemail", "yyds", "gptmail", "cfmail", "remail") else "gptmail"
...
def _remail_create(name, domain, expiry_ms, api_key, base_url, sess):
    """ReMail 下单租用邮箱。POST /v1/open/orders {projectId, productId} → deliveryEmail+serviceToken。"""
    base = _norm_base(base_url, REMAIL_BASE_URL)
    key = (api_key or REMAIL_API_KEY or "").strip()
    if not key:
        raise RuntimeError("ReMail 需要 REMAIL_API_KEY")
    headers = {"X-Api-Key": key, "Content-Type": "application/json"}
    payload = {
        "projectId": REMAIL_PROJECT_ID,
        "productId": REMAIL_PRODUCT_ID,
    }
    idem = uuid.uuid4().hex[:16]
    headers["Idempotency-Key"] = f"regfactory-{idem}"
    r = sess.post(f"{base}/v1/open/orders", json=payload, headers=headers, timeout=HTTP_TIMEOUT)
    if r.status_code >= 400:
        raise RuntimeError(f"ReMail create {r.status_code}: {r.text[:200]}")
    data = r.json() if r.content else {}
    email = data.get("deliveryEmail") or data.get("email") or ""
    token = data.get("serviceToken") or data.get("token") or ""
    oid = data.get("id") or data.get("orderNo") or email
    if not email:
        raise RuntimeError(f"ReMail create 返回缺少 email: {str(data)[:200]}")
    return {"id": str(oid), "email": str(email), "token": str(token), "provider": "remail", "raw": data}


def _remail_fetch(mailbox_id, email, token, api_key, base_url, sess):
    """ReMail 取信。GET /v1/pickup?email=...&token=... → {items:[{verificationCode,...}]}。"""
    base = _norm_base(base_url, REMAIL_BASE_URL)
    key = (api_key or REMAIL_API_KEY or "").strip()
    if not email or not token:
        return []
    headers = {"X-Api-Key": key} if key else {}
    r = sess.get(f"{base}/v1/pickup", headers=headers,
                 params={"email": email, "token": token}, timeout=HTTP_TIMEOUT)
    if r.status_code >= 400:
        return []
    data = r.json() if r.content else {}
    ...
    for raw in items:
        if not isinstance(raw, dict):
            continue
        item = dict(raw)
        # 归一字段：verificationCode 直接放顶层，subject/from 若有也保留
        if item.get("verificationCode") and not item.get("code"):
            item["code"] = item["verificationCode"]
        out.append(item)
    return out
